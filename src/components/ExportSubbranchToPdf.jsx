...
const ExportSubbranchToPdf = ({ konuBaslik, altKonuBaslik, altDallar }) => {
  ...
  const exportToPdf = async () => {
    ...
    try {
      ...
      const sanitizedKonuBaslik = turkishToEnglish(konuBaslik).replace(/[^\w\s]/gi, '');
      const sanitizedAltKonuBaslik = turkishToEnglish(altKonuBaslik).replace(/[^\w\s]/gi, '');
      doc.save(`${sanitizedKonuBaslik}_${sanitizedAltKonuBaslik}_Soru_Bankasi.pdf`);
    } catch (error) {
