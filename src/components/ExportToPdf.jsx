...
const ExportToPdf = ({ konuBaslik, altKonular }) => {
  const exportToPdf = () => {
    ...
    // Başlık bilgisi
    doc.setFontSize(18);
    doc.text(`${konuBaslik} - Soru Bankası`, 14, 20);
        ...
        Object.entries(altKonu.altdallar).forEach(([altDalId, altDal]) => {
          ...
          if (altDal.sorular) {
            Object.entries(altDal.sorular).sort((a, b) => {
              ...
              doc.text(`Doğru Cevap: ${soru.dogruCevap || ""} (${dogruCevapSik} şıkkı)`, 20, yPos);
              yPos += 6;
              
              // Açıklama
              if (soru.aciklama) {
                doc.setFont("helvetica", "italic");
                const aciklama = doc.splitTextToSize(`Açıklama: ${soru.aciklama}`, 170);
    ...
    // PDF'i indir
    doc.save(`${konuBaslik}_Soru_Bankasi.pdf`);
  };
